...
for (i in sims_to_run) {
  ...
  dat <- unname(as.matrix(read.csv(
    paste0("inst/Simulation_trajectories/sim_INARMA11_s", s, "_lgt_", lgt, "_nbin.csv")
  )))
  ...
  for (k in inds_to_run) {
    ...
    if (k %% 10 == 0) {
      ...
      write.csv(res, file = paste0("inst/Estimation/Results/Estim_MLE_s", s, "_", lgt, "_nbin.csv"),
                row.names = FALSE)
    }
  }
