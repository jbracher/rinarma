...
###############################################################################
## Inference for the INGARCH(1, 1) model using the approximation by an AR model
###############################################################################
  ...
  for (lgt in vals_lgt) {  # Loop over the series lengths
    ...
    for (k in 1:n_sim) {
      ...
      sim <- as.numeric(
        tsglm.sim(
          lgt,
          param = list(intercept = tau * (1 - sum(beta)),
    ...
    # Write the results
    write_csv(results_tab, file = paste0("inst/AR_approximation_example/Results/AR_INGARCH11_s", s, "_lgt", lgt, ".csv"))
  }
}
